...
public class LoadoutState : AState
{
    ...
    public VideoSequencer videoSequencer;
    ...
    private void OnEnable()
    {
	    ...
	    {
		    RectTransform rt = AISyncPanel.GetComponent<RectTransform>();
		    rt.DOScale(0f, .5f).OnComplete(() => AISyncPanel.SetActive(false));
	    }
	    videoSequencer.gameObject.GetComponent<Image>().DOColor(new Color(1f, 1f, 1f, 1f), .2f);
	    // TODO:If not play
    }
	...
	public void Invoke_AISync()
	{
		var rt = AISyncPanel.GetComponent<RectTransform>();
		rt.localScale = Vector3.zero;
		AISyncPanel.SetActive(true);
		MLModeController.instance.Set(MLModelModes.PoseChecker);
		rt.DOScale(1f, .5f).OnComplete(() => videoSequencer.gameObject.GetComponent<Image>().DOColor(new Color(1f, 1f, 1f, VideoSequencerTransparencyValue), 1f));
	}
}
